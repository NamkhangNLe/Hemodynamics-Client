...
export default function View() {
    ...
    useEffect(() => {
        async function fetchPatientCalculations() {
            ...
            const patientResponse = await fetch(`${process.env.REACT_APP_API_BASE_URL}/record/${id}`);
            ...
            const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/calculation/${id}`);
            if (!response.ok) {
    ...
    async function handleSave(e) {
        ...
        fetch(`${process.env.REACT_APP_API_BASE_URL}/updatecalc/${editingID}`, {
            method: "PATCH",
