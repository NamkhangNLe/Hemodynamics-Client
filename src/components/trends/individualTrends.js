...
const IndividualTrends = () => {
    ...
    useEffect(() => {
        ...
        async function getPatientObj() {
            const response = await fetch(`${process.env.REACT_API_BASE_URL}/record/${selectedPatientID}`);
            if (!response.ok) {
                const message = `An error occurred: ${response.statusText}`;
                window.alert(message);
                return;
    ...
    async function getCalculations() {
        const response = await fetch(`${process.env.REACT_API_BASE_URL}/calculation/${selectedPatientID}`);
